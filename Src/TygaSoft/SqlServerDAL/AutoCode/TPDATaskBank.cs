...
{
    public partial class TPDATaskBank : ITPDATaskBank
    {
        ...
        public int Insert(TPDATaskBankInfo model)
        {
            ...
            sb.Append(@"insert into TPDA_TaskBank (TB_cid,TB_date,TB_Rno,TB_BNO,TB_Bname,TB_Ptime,TB_boxNum,TB_notes,TB_oper,TB_opTime,TB_State,TB_Type,TB_Stime,TB_Etime,TB_Tlen,TB_KilM,TB_Fee,TB_Wxs,TB_Emp,TB_CNO)
			            values
						(@TBcid,@TBdate,@TBRno,@TBBNO,@TBBname,@TBPtime,@TBboxNum,@TBnotes,@TBoper,@TBopTime,@TBState,@TBType,@TBStime,@TBEtime,@TBTlen,@TBKilM,@TBFee,@TBWxs,@TBEmp,@TBCNO)
			            ");
...
new SqlParameter("@TBWxs",SqlDbType.Int),
...
new SqlParameter("@TBCNO",SqlDbType.VarChar,20)
            ...
            parms[17].Value = model.TBWxs;
            parms[18].Value = model.TBEmp;
            parms[19].Value = model.TBCNO;
        ...
        public int Update(TPDATaskBankInfo model)
        {
            ...
            sb.Append(@"update TPDA_TaskBank set TB_date = @TBdate,TB_Rno = @TBRno,TB_BNO = @TBBNO,TB_Bname = @TBBname,TB_Ptime = @TBPtime,TB_boxNum = @TBboxNum,TB_notes = @TBnotes,TB_oper = @TBoper,TB_opTime = @TBopTime,TB_State = @TBState,TB_Type = @TBType,TB_Stime = @TBStime,TB_Etime = @TBEtime,TB_Tlen = @TBTlen,TB_KilM = @TBKilM,TB_Fee = @TBFee,TB_Wxs = @TBWxs,TB_Emp = @TBEmp,TB_CNO = @TBCNO 
			            where TB_cid = @TBcid
					    ");
...
new SqlParameter("@TBCNO",SqlDbType.VarChar,20)
        ...
        public int Delete(string tBcid)
        {
            ...
            sb.Append("delete from TPDA_TaskBank where TB_cid = @TBcid ");
            SqlParameter[] parms = {
                                     new SqlParameter("@TBcid",SqlDbType.Char,2)
                                   };
            parms[0].Value = tBcid;
        ...
        public bool DeleteBatch(IList<object> list)
        {
            ...
            {
                n++;
                sb.Append(@"delete from TPDA_TaskBank where TBcid = @TBcid" + n + " ;");
        ...
        {
            TPDATaskBankInfo model = null;

            StringBuilder sb = new StringBuilder(300);
            sb.Append(@"select top 1 TB_cid,TB_date,TB_Rno,TB_BNO,TB_Bname,TB_Ptime,TB_boxNum,TB_notes,TB_oper,TB_opTime,TB_State,TB_Type,TB_Stime,TB_Etime,TB_Tlen,TB_KilM,TB_Fee,TB_Wxs,TB_Emp,TB_CNO 
			            from TPDA_TaskBank
						where TB_cid = @TBcid ");
            SqlParameter[] parms = {
                                     new SqlParameter("@TBcid",SqlDbType.Char,2)
        ...
        public IList<TPDATaskBankInfo> GetList(int pageIndex, int pageSize, out int totalRecords, string sqlWhere, params SqlParameter[] cmdParms)
        {
            ...
            sb.Append(@"select * from(select row_number() over(order by TB_cid) as RowNumber,
			          TB_cid,TB_date,TB_Rno,TB_BNO,TB_Bname,TB_Ptime,TB_boxNum,TB_notes,TB_oper,TB_opTime,TB_State,TB_Type,TB_Stime,TB_Etime,TB_Tlen,TB_KilM,TB_Fee,TB_Wxs,TB_Emp,TB_CNO
					  from TPDA_TaskBank ");
            if (!string.IsNullOrEmpty(sqlWhere)) sb.AppendFormat(" where 1=1 {0} ", sqlWhere);
            sb.AppendFormat(@")as objTable where RowNumber between {0} and {1} ", startIndex, endIndex);

            IList<TPDATaskBankInfo> list = new List<TPDATaskBankInfo>();
            ...
            using (SqlDataReader reader = SqlHelper.ExecuteReader(SqlHelper.QdhtyyDbConnString, CommandType.Text, sb.ToString(), cmdParms))
            {
                if (reader != null && reader.HasRows)
                {
                    while (reader.Read())
                    {
                        TPDATaskBankInfo model = new TPDATaskBankInfo();
                        ...
                        model.TBWxs = reader.IsDBNull(18) ? 0 : reader.GetInt32(18);
                        model.TBEmp = reader.IsDBNull(19) ? string.Empty : reader.GetString(19);
                        model.TBCNO = reader.IsDBNull(20) ? string.Empty : reader.GetString(20);
        ...
        {
            StringBuilder sb = new StringBuilder(500);
            sb.Append(@"select TB_cid,TB_date,TB_Rno,TB_BNO,TB_Bname,TB_Ptime,TB_boxNum,TB_notes,TB_oper,TB_opTime,TB_State,TB_Type,TB_Stime,TB_Etime,TB_Tlen,TB_KilM,TB_Fee,TB_Wxs,TB_Emp,TB_CNO
                        from TPDA_TaskBank ");
            if (!string.IsNullOrEmpty(sqlWhere)) sb.AppendFormat(" where 1=1 {0} ", sqlWhere);
