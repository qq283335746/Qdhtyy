    ...
    public partial class SiteMenus
    {
        ...
        public IList<SiteMenusInfo> GetMenusAccess(string appName, string[] accessIds, bool isAdministrators)
        {
            ...
            if(!isAdministrators)
            {
                ...
                var sqlWhere = string.Format("and AccessId in ({0}) ", sbIn.ToString().Trim(','));
            ...
            var parm = new SqlParameter("@ApplicationId", Guid.Parse(appId.ToString()));
                            ...
                            if (maList != null && maList.Count > 0)
                            {
                                ...
                                if(qumaInfo != null)
                                {
                                    ...
                                    if(maitemInfo != null)
                                    {
                                        ...
                                        if (maitemInfo.IsAdd) model.IsView = false;
                                        if (maitemInfo.IsEdit) model.IsView = false;
                                        if (maitemInfo.IsDelete) model.IsView = false;
                                    }
                                }
                            }
        ...
        public List<SiteMenusInfo> GetMenusAccess(object appId, Guid accessId,bool isAdministrators)
        {
            var list = new List<SiteMenusInfo>();
            var maInfo = new SiteMenusAccess().GetModel(accessId);
            List<SiteMenusAccessItemInfo> maitems = null;
